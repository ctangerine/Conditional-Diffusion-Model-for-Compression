...
def get_frames_and_diff_scores(frames: np.ndarray,
	...
	for i in tqdm(range(N), desc="Generating visualization video"):
		...
		# Calculate drop rate up to current frame
		dropped = i + 1 - len(keep_indices)
		drop_rate = dropped / (i + 1) if i > 0 else 0
		...
		# Draw vertical lines at kept frame indices
		for idx in keep_indices:
			ax2.axvline(x=idx, color='green', linestyle=':', alpha=0.7)
		ax2.set_title(f"Diff Score\nDrop Rate: {drop_rate:.2%}")
		ax2.set_xlim(0, N)
		ax2.set_ylim(0, max(diff_scores) + 10)
		ax2.legend()

		# Optionally, annotate the last point with drop rate
		ax2.annotate(f"{drop_rate:.2%}", xy=(i, diff_scores[-1]), xytext=(i, diff_scores[-1] + 5),
...
if __name__ == "__main__":
	...
	redundants, diff_scores = get_frames_and_diff_scores(frames, color_space='YUV', k=1.0, min_spacing=5,
														 output_path='diff_visualization2.mp4', fps=10)
